...
cd "$LIVE"
echo 'a0bf85aa3690137a25ae66c3660e263b02bd3d70e7b1826723127f6872573596  src/tutorials.js' | sha256sum -c -
echo '3adfb194bdeea10f6f0c8b95e777eb1506427687eb9a2711f9732e80054b07d6  src/routes/tutorials.js' | sha256sum -c -
...
test "$(wc -l < "$WORK/zip-files")" = 8
unzip -q "$ZIP" -d "$WORK/delta"
test -z "$(find "$WORK/delta" -type l -print -quit)"
(cd "$WORK/delta" && sha256sum -c SHA256SUMS)
for file in src/tutorials.js src/routes/tutorials.js migrations/012_tutorial_catalog.sql public/admin/index.html public/admin/admin.js public/admin/tutorial-publisher.js public/admin/tutorial-publisher.css; do
...
trap - ERR INT TERM
echo "DEPLOY_TUTORIAL_CATALOG_20260906_OK backup=$BACKUP previous=$PREVIOUS artifact_sha=$EXPECTED"
