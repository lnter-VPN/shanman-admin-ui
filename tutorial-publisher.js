...
window.TutorialPublisher=(()=>{
 ...
 const tutorialIcons=['rocket','robot','wand','folder','building','film','gear','mobile','cross'];
 Object.assign(icons,{rocket:'M14 4c3-1 6-1 6-1s0 3-1 6l-7 7-5-5 7-7M7 11H3l4-5 6-1M12 16v5l5-4 1-6M5 16l-2 5 5-2',robot:'M5 7h14v13H5zM12 3v4M9 11h.01M15 11h.01M9 16h6M2 11v5M22 11v5',wand:'m5 19 12-12 3 3L8 22zM6 3v6M3 6h6M18 16v6M15 19h6',building:'M4 21V7l8-4v18M12 9h8v12M2 21h20M7 8v2M7 13v2M7 18v2M16 12v2M16 17v2',film:'M3 9h18v12H3zM3 9V5l17-3 1 4-18 3M7 4l3 4M14 3l3 4',gear:'m9 3 6 0 1 3 3 1 2 5-2 5-3 1-1 3H9l-1-3-3-1-2-5 2-5 3-1zM12 8a4 4 0 1 0 0 8 4 4 0 0 0 0-8',mobile:'M7 2h10v20H7zM7 6h10M11 19h2',cross:'M9 3h6v6h6v6h-6v6H9v-6H3V9h6z'});
 function tutorialIcon(item){let hash=0;for(const char of String(item.id||item.slug||''))hash=(hash*31+char.charCodeAt(0))>>>0;const i=hash%tutorialIcons.length;return `<span class="tp-document-icon tp-tone-${i}">${icon(tutorialIcons[i])}</span>`}
 const statusName={draft:'草稿',published:'已发布',unpublished:'已下架'};
 const lines=value=>Array.isArray(value)?value.join('\n'):'';
 let root,api,base,token,requestAdvancedPassword,advancedPassword='',acceptedCategory='general',items=[],categories=[],current=null,dirty=false,busy=0,saving=false,filter={search:'',category:'',status:''};
 ...
 function dispose(){advancedPassword='';for(const url of blobUrls.values())URL.revokeObjectURL(url);blobUrls.clear();dirty=false;current=null;document.querySelectorAll('dialog[data-tp-dialog]').forEach(node=>node.remove())}
 window.addEventListener('beforeunload',event=>{if(dirty||busy||saving){event.preventDefault();event.returnValue=''}});
 async function mount(options){
  root=options.root;api=options.api;base=options.base;token=options.token;requestAdvancedPassword=options.requestAdvancedPassword;
 ...
 function list(){
  ...
  root.innerHTML=`<header class="tp-heading"><div><h2>让每一篇教程，更容易读懂</h2><p>按统一分类编写图文教程，确认后发布到客户端。</p></div><div class="tp-heading-actions"><button type="button" class="tp-button" data-tp="categories">${icon('folder')}管理分类</button><button type="button" class="tp-button tp-primary" data-tp="new">${icon('plus')}新建教程</button></div></header>
 ...
 function edit(item={}){
  current=structuredClone(item);dirty=false;advancedPassword='';acceptedCategory=item.category||'general';
 ...
 async function onChange(event){
  if(event.target.matches('[name=category]')){
   ...
   if(select.value==='advanced'){
    ...
    try{const secret=await requestAdvancedPassword?.();if(secret){advancedPassword=secret;acceptedCategory='advanced'}else select.value=acceptedCategory}
    catch(failure){select.value=acceptedCategory;error(failure.message)}
    finally{select.disabled=false}
   }else{acceptedCategory=select.value;advancedPassword=''}
   changed();return;
  }
  if(event.target.matches('[data-upload]')){void upload(event.target.closest('[data-image-zone]'),[...event.target.files]);event.target.value=''}else onInput(event)}
 ...
 function dialog(title,html){
  ...
  element.innerHTML=`<header><div><h2>${escape(title)}</h2><p>${title==='管理教程分类'?'分类与智能体超市、技能保持一致。':'核对内容与图片；预览不会更改发布状态。'}</p></div><button class="tp-button" type="button" data-dialog-close aria-label="关闭">关闭</button></header>${html}`;
 ...
 function manageCategories(){
  dialog('管理教程分类',`<div class="tp-category-manager"><p>教程统一使用下列分类。高级教程在编辑及发布时需验证密码，客户端也需解锁后查看。</p><div class="tp-catalog-list">${categories.map(category=>`<span>${escape(category.name)}<small>${items.filter(item=>item.category===category.id).length} 篇</small></span>`).join('')}</div></div>`);
 }
 ...
 async function submit(event){
  ...
  try{
   ...
   if(draft.category==='advanced'){if(!advancedPassword)advancedPassword=await requestAdvancedPassword?.()||'';if(!advancedPassword)return;draft.advancedCategoryPassword=advancedPassword}
