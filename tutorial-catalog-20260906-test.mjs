...
assert.ok(categories.items.some(x=>x.id==='general')&&categories.items.length===9);
...
if(process.env.PRODUCTION_CHECK_ONLY==='1'){
  ...
  try{
    ...
    await client.query("INSERT INTO tutorial_legacy_probe.tutorial_categories(id,name) VALUES('custom-general','通用')");
    await client.query(await fs.readFile('/app/migrations/012_tutorial_catalog.sql','utf8'));
    ...
    assert.equal(row.title,'legacy preserved');assert.equal(row.body,'');assert.equal(row.name,'通用');
  }finally{await client.query('ROLLBACK');client.release();await db.end()}
  const category={id:'general'};
  await call('/api/admin/tutorial-categories','POST',{name:'隔离验收分类'},true,409);
  await call(`/api/admin/tutorial-categories/${category.id}`,'PATCH',{name:'不允许变更'},true,409);
  ...
  assert.equal(published.items[0].body,draft.body);assert.equal(published.categories[0].name,'通用');
  ...
  await call('/api/admin/tutorials','POST',{...draft,slug:'advanced-probe',category:'advanced'},true,403);
  // The published transfer artifact must not contain a real category password.
  // Correct-password UI flows are verified locally. Seed a protected row only
  // inside this disposable database to verify production-route denial here.
  const advanced=await (await call('/api/admin/tutorials','POST',{...draft,slug:'advanced-probe'},true,201)).json();
  const protectedProbeDb=new Pool({connectionString:process.env.DATABASE_URL});
  try{await protectedProbeDb.query("UPDATE tutorials SET category='advanced' WHERE id=$1",[advanced.id])}
  finally{await protectedProbeDb.end()}
  await call(`/api/admin/tutorials/${advanced.id}/status`,'POST',{status:'published'},true,403);
  await call(`/api/admin/tutorials/${advanced.id}`,'PATCH',{title:'blocked without password'},true,403);
  await call(`/api/admin/tutorials/${advanced.id}`,'DELETE');
  await call(`/api/admin/tutorials/${item.id}`,'DELETE');
  await call(image.url,'GET',undefined,false,404);
  assert.equal((await (await call('/api/tutorials')).json()).items.length,0);
  console.log('ISOLATED_TUTORIAL_E2E_OK migrations legacy-category jsonb fixed-categories advanced-password image-auth publish edit unpublish delete invalid-image upload-limit');
}
